...
namespace QL_CUAHANGNOITHAT
{
    public partial class PNHoaDon : UserControl
    {
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            {
                HoaDon hoaDon = hd.FindHoaDon(txtMaHD.Text);

                // Kiểm tra xem hóa đơn có ở trạng thái chưa giao hay không\
                if (hoaDon != null && !hoaDon.TinhTrang.GetValueOrDefault())
                {
                    // Nếu chưa giao, tiến hành xóa
                    foreach (CTHoaDon cthd in hoaDon.CTHoaDons)
                    {
                        BLL_SanPham sp = new BLL_SanPham();
                        SanPham sanPham = sp.FindSanPham(cthd.MaSP);
                        sp.UpdateSoLuongTonByMaSP(cthd.MaSP, sanPham.SoLuongTon + cthd.SoLuong ?? 0);
                    }
                ...
                {
                    MessageBox.Show("Không thể xóa hóa đơn đã giao");
                }
            }
            else
            {
        ...
        private void dtHoaDon_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            // Kích hoạt nút tbnShow khi có dữ liệu
        ...
        {
            tbnShow.Enabled = !string.IsNullOrEmpty(txtMaHD.Text);
        }
